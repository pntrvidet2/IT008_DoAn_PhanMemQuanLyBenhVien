...
GO
 ...
 CREATE TABLE APPOINTMENT (
    ...
    -- Khóa ngoại trỏ tới ID Bác sĩ (nằm trong bảng Employee)
    CONSTRAINT FK_App_Doctor FOREIGN KEY (Doctor_ID) 
    REFERENCES EMPLOYEE(Emp_ID)
);
-- 1. Thêm lại cột App_ID với kiểu dữ liệu INT và tự động tăng (Identity)

-- Lưu ý: Kiểm tra tên bảng và cột của bà có khớp không nha (App_Date, Patient_ID, Doctor_ID, App_Note, Status)
-- 1. Thêm khóa ngoại trỏ tới bảng PATIENT
-- Đảm bảo cột Patient_ID ở hai bảng có cùng kiểu dữ liệu varchar(6)
ALTER TABLE APPOINTMENT
ADD CONSTRAINT FK_Ap_Patient 
FOREIGN KEY (Patient_ID) REFERENCES PATIENT(Patient_ID);

-- 2. Thêm khóa ngoại trỏ tới bảng EMPLOYEE (Bác sĩ)
-- Trong sơ đồ của bạn, Bác sĩ được quản lý trong bảng EMPLOYEE
ALTER TABLE APPOINTMENT
ADD CONSTRAINT FK_Ap_Doctor 
FOREIGN KEY (Doctor_ID) REFERENCES EMPLOYEE(Emp_ID);

-- 3. Thông báo kiểm tra
PRINT 'Đã tạo xong khóa ngoại cho bảng APPOINTMENT thành công!';
